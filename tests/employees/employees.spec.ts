...
test.describe.skip("Employees module", async () => {
  ...
  test.beforeAll(
    ...
    async ({ browser }) => {
      ...
      await page.getByRole("textbox", { name: "City" }).fill("test_city1");
      ...
      await page
        .getByRole("combobox")
        .selectOption({ label: "test_department1 — test_branch1" });
      ...
      await page
        .getByRole("combobox")
        .selectOption({ label: "test_department2 — test_branch1" });
      await page.getByRole("button", { name: "Create designation" }).click();
      await expect(page.getByText("Designation created")).toBeVisible();
    },
  );
  ...
  test.afterAll(
    "Remove the test branch, department and designations",
    async () => {
      await page.goto("/dashboard/designations");
  ...
  test.only("[EMP_01] Verify that user can create new employee.", async () => {
    ...
    await page
      .getByRole("textbox", { name: "Employee code *" })
      .fill("test_emp_code1");
    ...
    await page
      ...
      .fill("test_fname.test_lname@test.com");
    ...
    await page
      ...
      .first()
      .setInputFiles(
        "/home/sb/Downloads/Test files/Traveller/Traveller-DP.jpg",
      );
    // await page.getByRole("button", { name: "Upload" }).first().click();
    await page
      .locator('input[type="file"]')
